...
This is the authoritative Phase 8 specification, reconciled on 2026-09-07 from the historical
planning copy in `docs/recovered-planning/PHASE_08.md`. D-031/D-034 immutable sale-time cost
snapshots are the only approved basis for historical invoice profit; later supplier prices never
rewrite it.

Exact metric formulas, event/period dates, cost-coverage definitions, lifetime/favorite/late
metrics and small-cohort privacy thresholds remain `REVIEW_REQUIRED` at Gate G. A separately named
later-purchase-based estimate is omitted unless the owner explicitly approves it later.
...
Owner-only and computed exclusively from each canonical confirmed invoice revision's immutable
D-031/D-034 sale-time unit-cost snapshot and its compatible quantity/package/currency context.

Later linked purchases, latest supplier costs and changed preferred suppliers never recompute
historical invoice profit. A revision lacking a valid required snapshot is a data-integrity finding,
not permission to substitute a later price. The exact cost-coverage metric remains
`REVIEW_REQUIRED` at Gate G.

A separately named later-purchase-based estimate is excluded unless explicitly approved later.
...
Candidate initial periods — exact period set and event-date semantics are `REVIEW_REQUIRED`:
- per invoice;
- 30 days;
- 90 days;
- 1 year.
...
The groups below preserve the intended reporting surface. Every exact formula, inclusion rule,
event date, denominator and insufficient-data behavior must be locked at Gate G before its metric
is implemented; the names alone do not authorize an inferred calculation.
...
## Profit
- immutable D-031/D-034 sale-time revision-cost snapshot only;
- later purchase/latest cost never rewrites historical profit;
- missing required snapshot is a data-integrity finding, never a later-cost fallback;
- coverage correct;
- owner-only.
...
- historical profit only from immutable sale-time revision-cost snapshots; coverage definition approved;
