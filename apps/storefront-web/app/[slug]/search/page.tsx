...
export default async function SearchPage({ params, searchParams }: Props) {
  ...
  const products = q ? await fetchProducts(shop.slug, { query: q, page }) : { items: [], page: 1, page_size: 24, total: 0, has_more: false };
  const path = `/search?q=${encodeURIComponent(q)}`;
  return (
    <ShopFrame currentPath={`/${shop.slug}${path}${page > 1 ? `&page=${page}` : ""}`} lang={lang} query={q} shop={shop}>
