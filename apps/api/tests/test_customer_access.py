...
def test_link_gives_current_customer_pricing_without_exposing_anything_private(
    ...
    assert _catalog_prices(client, slug, secret)[1]["Cedar Water"] == (
        product["unit_price"],
        "personalized",
    )
    # Featured/recommended/detail honour the same context; the secret never reaches the logs.
    detail = client.get(
        f"/api/v1/public/{slug}/catalog/products/{product['id']}", headers={HEADER: secret}
    )
    ...
    with session_factory() as db:
        ...
        assert (
            stored is not None
            and stored.token_sha256 != secret
            and secret not in stored.token_sha256
        )
...
def test_rotation_revocation_one_active_and_isolation(client, session_factory):
    ...
    assert (
        client.get("/api/v1/public/customer-context", headers={HEADER: secret_a}).status_code == 404
    )
    assert (
        client.get("/api/v1/public/customer-context", headers={HEADER: secret_b}).status_code == 200
    )
    assert _catalog_prices(client, slug, secret_a)[1]["Cedar Water"][1] == "public", (
        "a rotated link is anonymous"
    )
    with session_factory() as db:
        active = db.scalar(
            select(func.count())
            .select_from(CustomerAccessLink)
            .where(CustomerAccessLink.revoked_at.is_(None))
        )
        assert active == 1
        assert (
            db.scalar(
                select(CustomerAccessLink).where(CustomerAccessLink.id == UUID(first["id"]))
            ).revoked_reason
            == "rotated"
        )
    ...
    assert (
        client.get("/api/v1/public/customer-context", headers={HEADER: secret_other}).json()[
            "display_name"
        ]
        == "Other Shop"
    )
    assert (
        client.get("/api/v1/public/customer-context", headers={HEADER: secret_b}).json()[
            "display_name"
        ]
        == customer["name"]
    )
    ...
    assert (
        client.get("/api/v1/public/customer-context", headers={HEADER: secret_b}).status_code == 404
    )
    assert (
        client.delete(
            f"/api/v1/tenants/{tenant}/customers/{customer['id']}/access-link", headers=_auth(token)
    ...
    assert (
        client.post(
            f"/api/v1/tenants/{tenant}/customers/{customer['id']}/access-link",
    ...
    assert (
        client.post(
            f"/api/v1/tenants/{other_tenant}/customers/{customer['id']}/access-link",
            headers=_auth(other_token),
        ).status_code
        == 404
    )
        ...
        actions = sorted(
            db.scalars(
                select(AuditEvent.action).where(
                    AuditEvent.tenant_id == UUID(tenant), AuditEvent.action.like("customer_%")
                )
            )
        )
        assert {
            "customer_link_issued",
            "customer_link_rotated",
            "customer_link_revoked",
            "customer_access_policy_changed",
        } <= set(actions)
...
def test_suspended_business_and_concurrent_issuance(client, session_factory):
    ...
    alive = [
        s
        for s in live
        if client.get("/api/v1/public/customer-context", headers={HEADER: s}).status_code == 200
    ]
    assert len(alive) == 1
    secret = alive[0]
    ...
    assert (
        client.post(
            f"/api/v1/platform/tenants/{tenant}/reactivate", headers=_auth(admin_token), json={}
        ).status_code
        == 200
    )
    assert (
        client.get("/api/v1/public/customer-context", headers={HEADER: secret}).status_code == 200
    )
    with session_factory() as db:
