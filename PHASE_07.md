...
This is the authoritative Phase 7 specification, reconciled on 2026-09-07 from the historical
planning copy in `docs/recovered-planning/PHASE_07.md`. D-021 owner-as-operator and least-privilege
rules remain binding.

Delivery-task terminal/reopen behavior, location replacement rules and the online routing/
geocoding provider remain `REVIEW_REQUIRED` at Gate F. A newly numbered decision/ADR must record
the provider; the historical `ADR-027` reference is invalid because D-027 governs PostgreSQL hosting.
...
A newly numbered routing-provider decision/ADR must be finalized immediately before Phase 7 using
then-current Lebanon coverage, pricing, quota, privacy and reliability. D-027 is already assigned
to PostgreSQL hosting and must not be reused.
...
The state names below are the preserved candidate minimum. Exact terminal/reopen behavior must be
resolved at Gate F before implementation.
...
The stored provenance fields and least-privilege rules are required. Exact replacement precedence
between confirmed/manual/GPS/geocoded readings remains `REVIEW_REQUIRED` at Gate F.
