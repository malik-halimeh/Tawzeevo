...
This is the authoritative Phase 4 specification, reconciled on 2026-09-07 from the historical
planning copy in `docs/recovered-planning/PHASE_04.md`. Higher-precedence decisions and contracts
continue to govern. Historical provenance remains in that directory.

The following details remain `REVIEW_REQUIRED` at Gate D and are not executable acceptance
criteria until explicitly resolved: exact pull page size, tombstone-retention duration, offline
lease duration/device retirement, Google OAuth scope/folder model, backup schedule/retention,
production KEK provider/recovery and exact DEK/KEK deployment. The values appearing below are
candidate defaults only where explicitly labelled as such.
...
Offline acceptance preserves D-038/D-039/D-043: one signed nonzero historical opening per
party/currency, separate operational supplier prepayment, nonnegative invoice inputs/totals and
strictly positive net sales for every confirmed revision. Rejected commands remain visible for
correction and are never silently coerced.
...
## Pull
Candidate initial maximum — `REVIEW_REQUIRED` at Gate D:
```text
500 changes/page
```
...
Candidate minimum — `REVIEW_REQUIRED` at Gate D:
```text
90 days
```
...
Candidate offline lease default is 24h; exact duration and retirement behavior are
`REVIEW_REQUIRED` at Gate D.
...
Use an application-managed Drive folder model per tenant/connected owner only after the exact
OAuth scope and folder model are approved at the P4-M5 precondition.

Candidate encryption deployment — `REVIEW_REQUIRED` before P4-M5:
- per-tenant DEK;
- AES-256-GCM payload;
