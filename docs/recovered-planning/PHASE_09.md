...
This was not original Phase 1 coursework, but is mandatory before production.
...
Initial staging targets, excluding external-provider latency where noted:
...
Before P9-M4, concrete hosting provider(s) must be selected/recorded.
...
## P9-M1 — Security matrix + password recovery
Implement/harden authorization/RLS, sessions, rate limits, brute-force/public abuse, CSP/CORS/CSRF/Origin, upload/dependency security, password recovery.
...
PRECONDITION: hosting/provider choices recorded.
...
- auth/session/password recovery hardened;
...
First milestone to implement: **P9-M1 — Security matrix + password recovery**
