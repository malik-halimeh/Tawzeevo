...
| A3 | F-005 | The hosting proxy topology is not visible from the repository. | Confirm in the hosting provider's documentation/dashboard that exactly one proxy hop sets `X-Forwarded-For` for the API service. `render.yaml` declares `TRUSTED_PROXY_HOPS=1`; change it only if the hop count really differs (`0` = ignore forwarded client-IP headers and key on the socket peer). **Never set a value higher than the number of proxies that actually append to `X-Forwarded-For`:** a too-high value makes the throttle key a value the caller put in the header itself, so the caller chooses its own bucket (the abuse F-005 closes). A too-low value is the safe error — it can collapse several visitors into one shared bucket, but never hands the choice to the caller. When the topology is uncertain, take the lower number. | Ten failed logins from one address throttle only that address (`/health/metrics` `auth_login_throttled` rises by one). | Note in the Phase 9 test report. | F-005 → FIXED_VERIFIED. |
