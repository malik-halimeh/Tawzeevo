...
| M3 | Pytest | `tests/test_order_workflow_continuation.py::test_payment_entered_from_the_newer_invoice_settles_exactly_that_invoice` | `python -m pytest "tests/test_order_workflow_continuation.py::test_payment_entered_from_the_newer_invoice_settles_exactly_that_invoice" -q` | Two outstanding invoices; the prefilled Choose-amounts receipt for the newer invoice's exact amount settles only the newer one | PASS / 2026-09-24 |
| M3 | Pytest | `tests/test_order_workflow_continuation.py::test_order_detail_links_its_delivery_and_foreign_invoices_are_not_readable` | `python -m pytest "tests/test_order_workflow_continuation.py::test_order_detail_links_its_delivery_and_foreign_invoices_are_not_readable" -q` | Order detail lists its delivery; another business's owner gets 404 for the invoice id | PASS / 2026-09-24 |
| M3 | Vitest (client) | `InvoiceEditor.continuation.test.tsx` (3 tests) | `npm exec --workspace @tawzeevo/operations-web -- vitest run src/components/InvoiceEditor.continuation.test.tsx` | Read-only document by link; payments prefill equals the hand-picked command; unknown/draft ids ignored | PASS / 2026-09-24 |
| M3 | Vitest (client) | `DeliveryPanel.test.tsx` › opened for an invoice | `npm exec --workspace @tawzeevo/operations-web -- vitest run src/components/DeliveryPanel.test.tsx` | Deliverable invoice preselected, same create command; unknown id ignored; entries link to their invoice | PASS / 2026-09-24 |
| M3 | Vitest (client) | `OrdersPanel.test.tsx` › a confirmed order offers next steps | `npm exec --workspace @tawzeevo/operations-web -- vitest run src/components/OrdersPanel.test.tsx` | Next-steps links (invoice, payment, delivery), existing sharing, delivery link | PASS / 2026-09-24 |
| M4 | Vitest (client) | `InvoiceSharing.test.tsx` › the issued share link can be copied | `npm exec --workspace @tawzeevo/operations-web -- vitest run src/components/InvoiceSharing.test.tsx` | Copy writes the issued URL; a clipboard failure explains the manual copy | PASS / 2026-09-25 |
| M4 | Vitest (client) | `InvoiceEditor.continuation.test.tsx` › Payments opened for the newer invoice (link assertion) | `npm exec --workspace @tawzeevo/operations-web -- vitest run src/components/InvoiceEditor.continuation.test.tsx` | An invoice payment line links to that invoice | PASS / 2026-09-25 |
| Demo | Playwright | `e2e/order-workflow-demo.spec.ts` | `npx playwright test e2e/order-workflow-demo.spec.ts` (in `apps/operations-web`, local stack running) | Link for X → X orders without name/phone → badge, `(1)` title and notice on another page → order already X's → confirm → share link works signed out and shows only that invoice → payment with the invoice preselected settles it → delivery with the invoice preselected → delivered → invoice opens from the delivery | PASS / 2026-09-25 |
| F-03 | Pytest | `tests/test_order_decline.py` (5 tests) | `python -m pytest tests/test_order_decline.py -q` | Public unlinked decline succeeds (DECLINED + CANCELLED, no ledger, leaves the RECEIVED set, repeat decline 409 with no change); linked decline unchanged; failure after the cancellation rolls everything back; an order stuck behind a cancelled invoice can be declined; approving a cancellation request on an unlinked received order works. 4 of 5 fail without the fix | PASS / 2026-09-25 |
| F-03 | Pytest | `tests/test_order_review.py::test_orders_awaiting_review_are_tenant_isolated_and_drop_after_a_decision` (workaround removed) | `python -m pytest "tests/test_order_review.py::test_orders_awaiting_review_are_tenant_isolated_and_drop_after_a_decision" -q` | A public order declined directly leaves the awaiting set | PASS / 2026-09-25 |
